---
title: Weekly Review 2026-07-27
tags:
  - ai
  - review
  - automation
...
status: active
type: index
summary: 試用フェーズ第7回週次レビュー — Open Questions 13件継続、未マージ報告3週、renderer-demos 追加
---
...
試用フェーズ第7回の週次レビューです。`docs/research/` 配下の Open Questions 13 件（運用モデル 4 + agent 調査 9）は前週から未変更で、主目的の決定が 7 週連続のボトルネックです。`main` には work 向け参照ノート `cost-disclosure-security-terminology.md` と `scripts/renderer-demos/`（描画エンジン比較デモ）が追加されましたが、新たな未決論点は増えていません。2026-06-29 / 07-13 / 07-20 の週次レビュー PR は依然 `main` 未マージのため、報告履歴に 3 週分の欠落があります。

## Open items

| 優先度 | ファイル | 論点 | 提案アクション |
|--------|----------|------|----------------|
| 高 | `docs/research/github-knowledge-limits.md` | knowledge-base の主目的はどれに近いか（長期保管庫 / 作業の外部記憶 / AI 参照セット） | **人間が今週決める** — 7 週連続未決。work / personal / agent 調査 / scripts の配置判断の根幹 |
| 高 | `docs/research/github-knowledge-limits.md` | 週次レビュー Automation + PR 報告の運用で十分か | 07-06 マージで実証は前進。未マージ報告 3 週で再発。マージ SLA と未マージブランチ整理を確定 |
...
| 中 | `docs/ai/reviews/` | 2026-06-29 / 07-13 / 07-20 の週次レビュー PR が `main` 未マージ | 一括マージするか、本報告（07-27）を正として古いブランチをクローズ |
...
1. **knowledge-base の主目的** — 長期保管庫 / 作業の外部記憶 / AI 参照セットのどれ（または組み合わせ）に寄せるか。7 週連続未決。`scripts/renderer-demos/` のような実行可能デモの置き場所判断もここに依存する。
2. **週次レビュー PR のマージ運用** — 未マージ報告が 06-29 / 07-13 / 07-20 の 3 週に拡大。Automation 成果物を毎週 `main` に残す手順（誰がいつマージするか、未マージブランチの整理方針）を確定する。
...
| 論点 | 関連ファイル | メモ |
|------|--------------|------|
| 運用モデル A/B/C | `trial-context.md` ↔ `github-knowledge-limits.md` Open Q1 | 7 週未決。agent 調査ノートの Phase 1/2 枠と整合 |
...
| 報告履歴の欠落 | `2026-06-22`（マージ済）↔ `2026-06-29` / `07-13` / `07-20`（未マージ）↔ 本報告 | 3 週分の報告が `main` に無い |
...
- [ ] 未決論点が増えたら `status: review` または `needs-discussion` タグを利用（検出精度向上）— `github-knowledge-limits.md` は候補
- [ ] `cursor/weekly-review-2026-06-17` / `2026-06-29` / `2026-07-13` / `2026-07-20` リモートブランチをクローズ（正本は 06-22 以降、未マージ分はマージ判断後）
...
- `docs/ai/reviews/2026-07-27-weekly-review.md` — 第7回週次レビュー報告（正本）
...
- Memories の過去実行メモ — 未マージブランチ・cron 整合の記録を参照できた

**検出しづらかった / 今回の学び:**

- **未マージ PR の存在** — `main` 上に 06-29 / 07-13 / 07-20 報告が無いことは、スキャンスクリプトだけでは分からない。`git fetch` + リモートブランチ `cursor/weekly-review-*` の確認が必須
- **報告履歴の欠落が拡大** — README は 07-06 までしか載っておらず、未マージ報告が 3 週に増加
...
- **repo 構成のドリフト** — `scripts/renderer-demos/`（npm 依存・3,600 行超）は Open Questions ではなく、`git log` と `scripts/README.md` の手動確認で発見。AGENTS.md の「アプリケーションコードを置かない」との整合を人間が判断すべき

**次回 Automation への改善案:**

- Memories に「未マージの `cursor/weekly-review-*` ブランチと README の最新日付を毎回照合」を維持
- 報告書に「前週からの diff」セクションを常設化（マージされたノート・未解決 OQ・未マージ PR・新規 scripts）
- `weekly_review_scan.py` に未マージ週次ブランチの検出ヒントを出すオプション追加を検討

## 前週からの diff（2026-07-06 マージ以降）

| 種別 | 内容 |
|------|------|
| マージ済みノート | `docs/work/cost-disclosure-security-terminology.md` — work 向けセキュリティ用語参照（Open Questions なし） |
| マージ済み基盤 | `AGENTS.md` — Cursor Cloud specific instructions 追加 |
| マージ済み scripts | `scripts/renderer-demos/` — 描画エンジン比較デモ（npm 依存あり） |
| 未解決 OQ | 13 件すべて継続（変更なし） |
| 未マージ報告 | `cursor/weekly-review-2026-06-29` / `2026-07-13` / `2026-07-20` |

## Next week

1. Top 3（主目的・PR マージ運用・agent Phase 2）について PR コメントまたはノートに決定を書く
2. 未マージの週次レビュー PR（06-29 / 07-13 / 07-20）を一括マージするか、本報告を正としてブランチをクローズするか判断
